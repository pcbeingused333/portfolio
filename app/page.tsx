...
const projects: Project[] = [
  ...
  },
  {
    ...
    description:
      "A Retrieval-Augmented Generation assistant over PDF documents: a LangGraph agent decides when to search, and every answer carries the source file and page number. Ships in two modes behind one flag — an in-memory FAISS demo that runs on a free 1 GB container with no database, and a pgvector-backed production path. It also ships with the harness that measures whether any of it works, and with CI that runs the whole suite on every push, including a headless boot of the app itself.",
    highlight:
      "The part I care about is the evaluation harness: 17 questions with known answers, five commands, and every number in the README regenerated by one of them rather than measured once by hand. It caught what tests could not. Production-sized chunks split the demo corpus into fewer pieces than the retrieval k, so recall read a perfect 100% while retrieval returned the whole document and filtered nothing — only rank-1 accuracy noticed. Spanish queries hit the right passage 0 times out of 4 against an English index, fixed by translating the retrieval query instead of paying 600 MB for a multilingual model. And running the real agent end-to-end found it refusing an ordinary customer question as off-topic without ever searching; correcting that took grounded retrieval from 16/17 to 17/17 and faithfulness from 0.87 to 0.96.",
